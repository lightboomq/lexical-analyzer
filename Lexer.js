...
const typeList = [
  ...
  {
    token: "keyword",
    regex: `^(map|filter|break|case|catch|class|constructor|const|continue|debugger|default|delete|do|else|export|extends|finally|for
...
function getHighlightToken(code, selectedToken, posToken) {
  const cleanCode = getReplacedHtmlMarkup(code);
  const span = `<span class='highlight'>${selectedToken}</span>`;
  let updateInput =
    cleanCode.slice(0, posToken - selectedToken.length) +
    span +
    cleanCode.slice(posToken);
